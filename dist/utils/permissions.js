...
class PermissionUtils {
     ...
     */
    static hasPermission(member, permissions, checkAdmin = true) {
        // If user is server owner, they have all permissions
        if (member.guild.ownerId === member.id) {
            return true;
        }
        // Check for admin permission if requested
        if (checkAdmin && member.permissions.has(discord_js_1.PermissionFlagsBits.Administrator)) {
            return true;
        }
     ...
     */
    static getReadablePermissionNames(permissions) {
        const nameMap = {
        ...
        permissions.forEach(perm => {
            ...
            if (typeof perm === 'bigint') {
                ...
                for (const [key, value] of Object.entries(discord_js_1.PermissionFlagsBits)) {
                    if (value === perm) {
